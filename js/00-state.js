...
const BASE_COA = {
  Assets: {
    ...
    accts: [
      ...
      {
        code: "1302",
        name: "Inventory - Housekeeping Supplies",
        type: "debit",
      },
  ...
  Revenue: {
    ...
    accts: [
      ...
      {
        code: "4301",
        name: "Loyalty Redemption Discount (contra)",
        type: "debit",
      },
...
const DEFAULT_CASH_CATEGORIES = [
  ...
  {
    label: "Room service income",
    dir: "in",
    account: "Food & Beverage - Room Service",
    reg: true,
    custom: false,
  },
  {
  ...
  },
  {
    ...
    dir: "in",
    account: "Food & Beverage - Restaurant",
    reg: false,
    custom: false,
  },
  {
    label: "Events / banquet income",
    dir: "in",
    account: "Conference & Events Revenue",
    reg: false,
    custom: false,
  },
  {
  ...
  },
  {
    label: "Supplies",
    dir: "out",
    account: "Room Supplies & Amenities",
    reg: true,
    custom: false,
  },
  {
    label: "Food & drink stock",
    dir: "out",
    account: "F&B Cost of Goods Sold",
    reg: true,
    custom: false,
  },
  {
  ...
  {
    label: "Staff wages / salary",
    dir: "out",
    account: "Salaries - Management",
    reg: false,
    custom: false,
  },
  {
  ...
  },
  {
    ...
    dir: "out",
    account: "Website & Online Platforms",
    reg: false,
    custom: false,
  },
  {
...
if (
  coa["Equity"] &&
  !coa["Equity"].accts.some((a) => a.name === "Owner's Drawings")
) {
  coa["Equity"].accts.push({
    code: "3004",
...
//     saveCashCategories();
// }
// Migration & setup block for cash categories
let capitalCat = cashCategories.find((c) => c.label === "Owner's Capital");
if (capitalCat) {
...
if (!Array.isArray(redeemRules.rewards) || !redeemRules.rewards.length)
  redeemRules.rewards = DEFAULT_REDEEM_RULES.rewards.slice();
...
function migrateCoa() {
  ...
  if (
    ...
    !coa["Operating Expenses"].accts.find((a) => a.name === "Cash Over / Short")
  ) {
    coa["Operating Expenses"].accts.push({
      code: "6802",
...
function saveCoa() {
  try {
    localStorage.setItem("hms_coa", JSON.stringify(coa));
  } catch (e) {}
}

function saveCashCategories() {
  try {
    localStorage.setItem("hms_cash_categories", JSON.stringify(cashCategories));
  } catch (e) {}
}

function saveSubCategories() {
  try {
    localStorage.setItem("hms_subcategories", JSON.stringify(subCategories));
  } catch (e) {}
}

function saveRedeemRules() {
  try {
    localStorage.setItem("hms_redeem_rules", JSON.stringify(redeemRules));
  } catch (e) {}
}
...
function saveCompanies() {
  try {
    localStorage.setItem("hms_companies", JSON.stringify(companies));
  } catch (e) {}
}
...
function methodAccountName(method, cashType) {
  switch (method) {
    ...
    case "company":
      return "Accounts Receivable - Corporate";
    default:
      return cashType === "petty" ? "Petty Cash" : "General Cash";
  }
}
...
function methodLabel(method, cashType) {
  switch (method) {
    ...
    default:
      return cashType === "petty" ? "Petty" : "General";
  }
}
...
function sectionTotalRange(sectionName, orientation, from, to, includeOpening) {
  ...
  let total = 0;
  coa[sectionName].accts.forEach((a) => {
    total += accountNetRange(a, orientation, from, to, includeOpening);
  });
  return total;
}
...
function sectionTotal(sectionName, orientation) {
  ...
  let total = 0;
  coa[sectionName].accts.forEach((a) => {
    total += accountNet(a, orientation);
  });
  return total;
}
...
function getReceivables() {
  ...
  open.forEach((t) => {
    const key = t.isCompany
      ? "co:" + (t.companyId || t.companyName)
      : t.nonMember
        ? "nm:" + (t.nonMemberName || "Non-member")
        : "g:" + t.gId;
    if (!groups[key]) {
      groups[key] = {
        name: bookingGuestName(t),
        kind: t.isCompany ? "Company" : t.nonMember ? "Non-member" : "Member",
        total: 0,
        current: 0,
        d30: 0,
        d60: 0,
